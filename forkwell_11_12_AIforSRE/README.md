...
　「AIが人間の仕事を奪う」と言われて久しいですが、今後も残っていく仕事にはどのようなものがあるでしょうか？例えば、工場の生産ラインはより高度に機械化され、センサー技術とAIの組み合わせによってさらなる自動化が進むと予想されます。それでは、私たちITエンジニアにはどのような未来が待っているのでしょうか？

### 出力の真贋を見極める

　普段のコーディングやQA業務において、AIが時に誤った回答をすることがあります。これは、存在しない情報を答えてしまう「ハルシネーション（幻覚）」が原因の一つです。なぜこのような現象が起こるかというと、LLMは確率をもとに結果を生成しているため、文脈にそぐわない回答や誤った情報を提示することがあるのです。このため、LLMを活用する際には、出力された情報の真贋を見極める力が必須です。

　かつて2ちゃんねる創設者の西村ひろゆき氏が「嘘を見抜けないとインターネットを使うのは難しい」と言ったように、AIを「道具」として使うためには、正しい知識を持つことが不可欠です。正しい知識がなければ、どれほど優れた道具であってもその力を十分に引き出すことはできません。

### LLMのグルー実装としての人類

　現状、LLMが能動的に問題を発見し、独自に行動する段階には達していません。多くの場合、人が何かしらの入力を与え、AIがその結果を出力し、それを受け取った人が次の行動を決定する、といったモデルに依存しています。このような状況を見ていると、まるで「AIをつなげるために人間が存在している」かのようです。これは一種の過渡期であり、今後センサー技術やシーケンサーのような処理系が発展すれば、AIが能動的に活動する未来も遠くはないでしょう。
...
　LLMの進化に伴い、エンジニアとしてどのように対応していくべきかについても触れていきます。

### 継続的な基礎知識の研鑽

　LLMの活用が進む中で、エンジニアに求められるのは、AIをただ使うだけでなく、その出力を評価し、誤りを見抜く能力です。LLMはどれほど高性能でも、時に誤った情報を出力することがあるため、基礎的な知識や理論を継続的に学び直し、道具としてのAIを使いこなす力が不可欠です。

### プラットフォームとしてのLLM活用

　今後LLMがプラットフォームとして広く利用されることに伴い、それを活用するスキルが重要になります。iOSやAWSが登場した際と同様に、LLMを自分の開発環境や業務に統合し、最大限に活用するための技術を身に付けることが、エンジニアの競争力に直結するでしょう。プラットフォームとしてのLLMの特性を理解し、それを活かしたアプリケーションやシステム構築に取り組むことが求められます。

### 情報の真贋を見る力を養う

　最後に、LLMが生成した情報の真贋を見極める力を養うことも不可欠です。AIによる情報が増加する中で、誤情報やフェイクが含まれていないかを判断する力が求められます。LLMを活用しながらも、その出力を客観的に評価し、信頼できる情報とそうでない情報を区別する能力を高めることが、エンジニアの生存戦略として重要です。
