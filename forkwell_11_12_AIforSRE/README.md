...
### SUZURIのスリスリくんチャット
　GMOペパボが提供するSUZURIでは、キャラクター「スリスリくん」と会話しながら、ユーザーに適した商品を提案する機能があります。内部では、RAGを活用し、ユーザーに適切な検索ワードを生成、それに基づいて検索結果を推薦する構造になっています。(なお現在は提供停止)
## ユーザーの入力補助
...
　LLMの発展を見ていると、Kubernetesが流行り始めた頃を思い出します。Kubernetesの採用が進むにつれて、「ある程度の規模ならKubernetesで十分」という流れが定着したように、LLMも今、似たような普及段階に差し掛かっているように感じます。その規模感の中で、すべてが自分の手の届かないところで決まっていく感覚には、少し距離を感じざるを得ません。技術がプラットフォーム化していく中で、ただの利用者にとどまるのか、プラットフォーム自体を作る側に回るのか、もしくはその発展に貢献するのか、エンジニアとしての立場が問われる場面が増えているように思います。

　例えば近年ではカヤックの[@fujiwara](https://x.com/fujiwara)が自身で作るOSSを「隙間家具」と位置づけて、開発しています。これはAWSのプラットフォームで現状では足りない機能や、自身の用途を満たせていない部分をOSSとして開発して、それがマネージドで対応されたら捨れるような適切な抽象度、サイズで開発されています。これも1つのプラットフォームを上手に使う例だと思います。

　私自身は、Kubernetes上で動く仮想待合室の仕組みや、OpenStackのLBを利用したアクセス制御の仕組みなどを作ってきました。私の場合はfujiwaraさんと少し立場が違い、オンプレミスのマルチテナント環境であるがゆえに、ある程度、ソフトウェアを自分で作ったほうがコスト面での優位性や事業差別化になるケースが多かったことからこのような取り組みをしてきました。

 LLMのプラットフォーム上でも、技術力をもとに上手に使う、差別化するという構図はあまり変わらないように現状では感じているため、今後もこのプラットフォームの上でもっとおもしろい何かを作るということは変わらないと考えています。
...
　また真贋を見極めるためのテクニックの１つとして、自分の知っている領域と紐づけるというテクニックがあります。以前、話してはRubyであるメソッドの実装の詳細を追う機会がありました。その時、私自身はCRubyのファイル構成や実装の詳細について詳しくはありませんでしたが、C言語を読み書きできる能力と、Rubyを読み書きできる能力は有していました。そこで、LLMを活用してCRubyの実装についてあたりをつけて、C言語のファイルを読み解き、Rubyの実装をもとに検証するということをしたことがあります。このようにすべてを自分で知らなくてもLLMを活用することで、自分の土俵で真贋を見極めるというようなテクニックも有効です。

　最後に、何かを類推し価値を判断する力や視点は一日にして身につくものではなく、簡単に習得できるものでもありません。日々自分の判断基準をアップデートし、自分なりの定規を磨き続けることを大切にしていきたいと思っています。
